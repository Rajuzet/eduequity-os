...
async function proxyRequest(
  ...
  // request.nextUrl.search already includes the leading "?" — use it directly
  const search = request.nextUrl.search || ''
  const targetUrl = `${API_URL}/api/v1/${pathStr}${search}`
  ...
  try {
    ...
    // Create response and forward cookies properly
    // Use getSetCookie() to handle multiple Set-Cookie headers (Node 18+)
    const newResponse = new NextResponse(responseText, {
      status: response.status,
      headers: response.headers,
    })

    // Forward all Set-Cookie headers — getSetCookie returns array of all cookies
    const setCookies = response.headers.getSetCookie?.() ?? []
    if (setCookies.length > 0) {
      // Remove any single set-cookie that was auto-copied, then re-add all
